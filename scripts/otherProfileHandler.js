...
function populatePostCards(data){
    ...
    for(let i = 0; i < data.length; i++){
        ...
        // fallback to a generic image if link is broken or empty
...
function populateOtherProfileData() {
    ...
    const defaultSrc = '../assets/images/default-profile.jpeg';
    const otherImg = document.getElementById('other-profile-image');
    if (otherImg) {
        otherImg.onerror = function(){ this.onerror = null; this.src = defaultSrc; };
        if (profileUrl && profileUrl !== 'null' && profileUrl !== 'undefined' && profileUrl.trim() !== '') {
            otherImg.src = profileUrl;
        } else {
            otherImg.src = defaultSrc;
        }
    }
